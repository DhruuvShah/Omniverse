        ...
        <div id="mobileMenuContent" class="hidden lg:flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-6 lg:gap-8 w-full max-w-screen-2xl mx-auto px-6 md:px-12 py-6 lg:py-4 absolute lg:static top-full left-0 glass-nav lg:bg-transparent shadow-2xl lg:shadow-none border-b border-apple-border lg:border-none z-40 max-h-[calc(100vh-70px)] overflow-y-auto lg:overflow-visible lg:max-h-none">
            ...
            <div class="w-full lg:w-auto flex flex-col lg:flex-row justify-start lg:justify-start lg:overflow-x-auto hide-scroll shrink-0">
                <div class="bg-apple-card border border-apple-border p-2 lg:p-1.5 rounded-2xl lg:rounded-full flex flex-col lg:flex-row gap-2 lg:gap-1 w-full lg:w-max">
                    <a href="index.html" class="px-5 py-3 lg:py-2 rounded-xl lg:rounded-full text-base lg:text-sm font-semibold transition-all whitespace-nowrap ACTIVE_TAB_COLOR text-white shadow-sm text-center lg:text-left">Marvel Movies</a>
                <a href="marvel-comics.html" class="px-5 py-3 lg:py-2 rounded-xl lg:rounded-full text-base lg:text-sm font-semibold transition-all whitespace-nowrap text-apple-muted hover:text-white hover:bg-[#2c2c2e]/70 text-center lg:text-left">Marvel Comics</a>
                <a href="dc-movies.html" class="px-5 py-3 lg:py-2 rounded-xl lg:rounded-full text-base lg:text-sm font-semibold transition-all whitespace-nowrap text-apple-muted hover:text-white hover:bg-[#2c2c2e]/70 text-center lg:text-left">DC Movies</a>
                <a href="dc-comics.html" class="px-5 py-3 lg:py-2 rounded-xl lg:rounded-full text-base lg:text-sm font-semibold transition-all whitespace-nowrap text-apple-muted hover:text-white hover:bg-[#2c2c2e]/70 text-center lg:text-left">DC Comics</a>
                </div>
            </div>
    ...
    <script>
        ...
        window.addEventListener('resize', () => {
            if (window.innerWidth >= 1024) { // lg breakpoint
                ...
                // If menu was closed on mobile, keep it hidden
...
for (const file of files) {
    ...
    // Restore classes
    content = content.replace(/bg-apple-card/g, 'bg-[#1c1c1e]');
    content = content.replace(/bg-apple-bg/g, 'bg-[#000000]');
    content = content.replace(/bg-apple-border/g, 'bg-[#3a3a3c]'); // The active tab border/bg is #3a3a3c usually
    content = content.replace(/text-apple-text/g, 'text-white'); // Not perfect but brings back most
    content = content.replace(/text-white hover:bg-blue-600/g, 'text-white hover:bg-blue-600');
    content = content.replace(/from-apple-card/g, 'from-[#1c1c1e]');
    content = content.replace(/to-apple-bg/g, 'to-[#0f0f10]');
    content = content.replace(/border-apple-border/g, 'border-[#333336]');
    content = content.replace(/bg-apple-hover/g, 'bg-[#2c2c2e]');
    ...
    content = content.replace(/text-transparent bg-clip-text bg-gradient-to-r from-apple-text via-apple-text to-apple-muted/g, 'text-transparent bg-clip-text bg-gradient-to-r from-white via-white to-white/50');
    
    // Some specific border/glass fixes
    content = content.replace(/background: var\(--apple-glass\);/g, 'background: rgba(0, 0, 0, 0.7);');
    content = content.replace(/background: var\(--apple-card\);/g, 'background: rgba(28, 28, 30, 0.85);');

    // Revert Nav
    let navReverted = originalNav;
    // Set active tab logic correctly for the file
    const isMarvelMovies = file === 'index.html';
    const isMarvelComics = file === 'marvel-comics.html';
    const isDCMovies = file === 'dc-movies.html';
    const isDCComics = file === 'dc-comics.html';

    let tabsHTML = \`<a href="index.html" class="px-5 py-3 lg:py-2 rounded-xl lg:rounded-full text-base lg:text-sm font-semibold transition-all whitespace-nowrap \${isMarvelMovies ? 'bg-[#3a3a3c] text-white shadow-sm' : 'text-apple-muted hover:text-white hover:bg-[#2c2c2e]/70'} text-center lg:text-left">Marvel Movies</a>
                <a href="marvel-comics.html" class="px-5 py-3 lg:py-2 rounded-xl lg:rounded-full text-base lg:text-sm font-semibold transition-all whitespace-nowrap \${isMarvelComics ? 'bg-[#3a3a3c] text-white shadow-sm' : 'text-apple-muted hover:text-white hover:bg-[#2c2c2e]/70'} text-center lg:text-left">Marvel Comics</a>
                <a href="dc-movies.html" class="px-5 py-3 lg:py-2 rounded-xl lg:rounded-full text-base lg:text-sm font-semibold transition-all whitespace-nowrap \${isDCMovies ? 'bg-[#3a3a3c] text-white shadow-sm' : 'text-apple-muted hover:text-white hover:bg-[#2c2c2e]/70'} text-center lg:text-left">DC Movies</a>
                <a href="dc-comics.html" class="px-5 py-3 lg:py-2 rounded-xl lg:rounded-full text-base lg:text-sm font-semibold transition-all whitespace-nowrap \${isDCComics ? 'bg-[#3a3a3c] text-white shadow-sm' : 'text-apple-muted hover:text-white hover:bg-[#2c2c2e]/70'} text-center lg:text-left">DC Comics</a>\`;

    navReverted = navReverted.replace(/<a href="index\.html"[\s\S]*?<\/a>/, tabsHTML); // Replace the whole block of links... wait regex won't match easily.
    // Instead, I'll just hard replace the tabs Group
    
    const tabsRegex = /<div class="bg-\[\#1c1c1e\].*?p-2 lg:p-1\.5 rounded-2xl lg:rounded-full flex flex-col lg:flex-row gap-2 lg:gap-1 w-full lg:w-max">[\s\S]*?<\/div>/;
    navReverted = navReverted.replace(tabsRegex, \`<div class="bg-[#1c1c1e] border border-[#333336] p-2 lg:p-1.5 rounded-2xl lg:rounded-full flex flex-col lg:flex-row gap-2 lg:gap-1 w-full lg:w-max">\n\${tabsHTML}\n</div>\`);
    
    // Replace Nav Block
    content = content.replace(/<nav class="sticky top-0 z-50 glass-nav border-b border-\[\#333336\] mb-8">[\s\S]*?<\/nav>/, navReverted);
    ...
    // Manual fallback for header
    content = content.replace(/<h1 class="text-4xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white via-white to-white\/50 tracking-tighter mb-4">/, '<h1 class="text-4xl md:text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white via-white to-white/50 tracking-tighter mb-4">');
